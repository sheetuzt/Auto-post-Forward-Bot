...
# --- BAN & AUTH SYSTEM ---
def is_banned_db(uid):
    return banned_col.find_one({"user_id": uid}) is not None

def ban_user_db(uid):
    banned_col.update_one({"user_id": uid}, {"$set": {"user_id": uid}}, upsert=True)
...
def full_remove_user_db(uid):
    ...
    targets_col.delete_many({"user_id": uid})
    sources_col.delete_many({"user_id": uid})
    settings_col.delete_many({"user_id": uid})
    count_col.delete_many({"user_id": uid})
    history_col.delete_many({"user_id": uid})
...
def add_target(uid, tid): targets_col.update_one({"target_id": tid, "user_id": uid}, {"$set": {"target_id": tid, "user_id": uid}}, upsert=True)
def remove_target(uid, tid): targets_col.delete_one({"target_id": tid, "user_id": uid})
